...
from utils.tools import EarlyStopping, adjust_learning_rate, save_model, load_model, My_loss
...
#             in_dim = 7
        ...
        model = SCINet(
            ...
            # input_len = 48
            # input_dim = 1
            # hid_size = 4.0  fai函数第一个conv1d中out_channels为hid_size*C,C为特征维度
            # num_stacks = 1
            # num_levels = 3  SCINet中SCI-block的层数
            # num_decoder_layer = 1  ?
            # concat_len = 0  ?
            # groups = 1  fai函数两个conv1d中的group参数
            # kernel = 5  fai函数第一个conv1d中kernel的维度
            # dropout = 0.25
    ...
    def _get_data(self, flag):
        ...
        data_set = Data(
            ...
            cols=args.cols,
            # cy_addcode
            scaler_way = args.scaler_way # 默认为StandardScaler
        )
...
#         return criterion
    ...
    def valid(self, valid_data, valid_loader, criterion):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(valid_loader):
            ...
            # cy_addcode:
            batch_x_cy = batch_x[:, -self.args.label_len:, :].cuda()
            batch_x_cy = valid_data.inverse_transform(batch_x_cy)
            prev = batch_x_cy
            ...
            if self.args.stacks == 1:
                ...
                # 改为：
                loss = criterion(pred.detach().cpu(), true.detach().cpu(), prev.detach().cpu(), self.args.loss)
                ...
                # 改为：
                loss = criterion(pred.detach().cpu(), true.detach().cpu(), prev.detach().cpu(), self.args.loss) + criterion(mid.detach().cpu(), true.detach().cpu(), prev.detach().cpu(), self.args.loss)
    ...
    def train(self, setting):
        ...
        for epoch in range(epoch_start, self.args.train_epochs):
            ...
            for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(train_loader):
                ...
                # cy_addcode:
                batch_x_cy = batch_x[:, -self.args.label_len:, :].cuda()
                batch_x_cy = train_data.inverse_transform(batch_x_cy)
                prev = batch_x_cy
                ...
                if self.args.stacks == 1:
                    ...
                    # 改为：
                    loss = criterion(pred, true, prev, self.args.loss)
                elif self.args.stacks == 2:
                    # 源码：
                    # loss = criterion(pred, true) + criterion(mid, true)
                    # 改为：
                    loss = criterion(pred, true, prev, self.args.loss) + criterion(mid, true, prev, self.args.loss)
                else:
                    print('Error!')
    ...
    def test(self, setting, evaluate=False):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(test_loader):
            ...
            # cy_addcode
            batch_x_cy = batch_x[:, -self.args.label_len:, :].cuda()
            batch_x_cy = test_data.inverse_transform(batch_x_cy)
            inputs.append(batch_x_cy.detach().cpu().numpy())
        ...
        inputs = np.array(inputs) # cy!
        inputs = inputs.reshape(-1, inputs.shape[-2], inputs.shape[-1]) # cy! 
        # result save
        if self.args.save:
            ...
            mae, mse, rmse, mape, mspe, corr = metric(preds, trues)
            maes, mses, rmses, mapes, mspes, corrs = metric(pred_scales, true_scales) # cy!
            ...
            np.save(folder_path + 'metrics_scales.npy', np.array([maes, mses, rmses, mapes, mspes, corrs])) # cy!
